...
import ru.netology.DGrigoryev.service.StatementService;
...
public class OperationController {
    ...
    @PostMapping(path = "/")
    public ResponseEntity<Object> saveOperation(@RequestBody Operation op) {
        List<Operation> operations = statementService.saveOperation(op);

        URI location = ServletUriComponentsBuilder.fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(op.getCustomerID())
                .toUri();
